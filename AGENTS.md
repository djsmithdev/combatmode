...
- `CombatMode/UI/Options/`: standalone custom options window.
  - `UI/Options/Draw.lua`: theme tokens `CM.UI.Colors` / `UI.Fonts` / `UI.Radius` (minimal grey ramp + single accent yellow), `UI.StripColors`, `UI.SetFontSize`, drawing primitives, and themed config tooltips (`UI.ShowTooltip` / `UI.HideTooltip` / `UI.AttachTooltip`).
  ...
  - `UI/Options/SpellMultiSelect.lua`: `UI.MakeSpellMultiSelect` / `ctx:SpellMultiSelect` for Reticle Targeting spell CSV lists (pills + spellbook suggestions).
