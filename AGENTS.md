...
- `CombatMode/Core/`: runtime behavior modules, grouped by domain.
  ...
  - `Core/Runtime/CVarManager.lua`: all CVar-writing helpers, reticle preset resolution (`CM.GetEffectiveReticleTargetingCVarValues` merges `CM.Constants.ReticleTargetingCVarValues` with account-wide `CM.DB.global.reticleTargetingCVarOverrides`; excluded keys in `CM.Constants.ReticleTargetingCVarEditorExcluded` are pruned and never overridden), and reset-to-default.
  - `Core/Runtime/BindingQueue.lua`: combat-safe deferred binding updates.
  - `Core/Runtime/Bootstrap.lua`: startup sequence (`CM.BootstrapFeatureModules`).
  ...
  - `Core/HealingRadial/HealingRadial.lua`: party radial.
