...
| **CombatMode/Core/** | Runtime behavior, grouped by domain: `Runtime/` (shell, events, CVars, binding queue, bootstrap), `FreeLook/` (mouselook + auto cursor unlock), `Crosshair/` (reticle, Interaction HUD with Left/Right side, Assisted Highlight with Left/Right side + own cast motion, animations for crosshair cast feedback), `ClickCasting/` (binding overrides, macro builder, addon bar resolver), `PartyRadial/` (party radial; `CM.PartyRadial` API). |
