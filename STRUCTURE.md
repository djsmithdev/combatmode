...
| **CombatMode/Core/** | Runtime behavior, grouped by domain: `Runtime/` (shell, events, CVars, binding queue, bootstrap), `FreeLook/` (mouselook + auto cursor unlock), `Crosshair/` (reticle, `InteractionHUD/` Target→Visual→HUD, `AssistedHighlight/` Keybinds→Motion→CastProgress→Feedback→Assist, animations for crosshair cast feedback), `ClickCasting/` (binding overrides, macro builder, addon bar resolver), `PartyRadial/` (party radial; `CM.PartyRadial` API). |
