...
--  Options preview simulates a full party including low-health, dead (Disabled atlas),
--  and mind-controlled (DeclineMark X fading over the role icon) slices so Visual
--  Settings show those states.
--  (melee vs ranged DPS via spec/class + NotifyInspect, Disabled when dead/ghost,
--  Decline when charmed/possessed) plus a low-health circle glow drawn on top of /
--  inside the role icon (same HB_LOW_PCT threshold as the bar glow).
--  Show/hide fades mainFrame alpha (combat-safe; never Show/Hide secure children for
--  visibility).
...
--    • Role atlases / ranged-spec tables live in Constants/PartyRadial.lua.
...
local CONTROLLED_OVERLAY_PULSE_PERIOD = 0.55
...
local function UpdateAllHealthBarGlowPulses(elapsed)
  ...
  RadialState.controlledOverlayPulsePhase = (RadialState.controlledOverlayPulsePhase or 0)
    + elapsed / CONTROLLED_OVERLAY_PULSE_PERIOD
  if RadialState.controlledOverlayPulsePhase >= 1 then
    RadialState.controlledOverlayPulsePhase = RadialState.controlledOverlayPulsePhase
      - math.floor(RadialState.controlledOverlayPulsePhase)
  end
  local controlledWave = 0.5 - 0.5 * math.cos(RadialState.controlledOverlayPulsePhase * math.pi * 2)
...
local function RequestPartySpecInspects()
  ...
  for _, member in ipairs(RadialState.partyData or {}) do
...
local function ResolveRoleIconAtlas(role, unitId, classFile, isPlaceholder, memberData)
  ...
  if
    (RadialState.optionsPreviewActive and memberData and memberData.previewControlled)
  ...
  local isDead = false
  if RadialState.optionsPreviewActive and memberData and memberData.previewDead then
    isDead = true
...
local function UpdateRoleIconLowHealthGlow(slice, memberData, isPlaceholder, hasRole)
  ...
  if RadialState.optionsPreviewActive or isPlaceholder then
