...
function CM.ApplyCVarConfig(info)
  ...
  elseif CVarType == "blizzard" then
    ...
    CM.DebugPrint("Invalid CVarType in CM.ApplyCVarConfig for " .. FeatureName .. ": " .. tostring(CVarType))
    return
  end
...
local function ShouldFreeLookBeOff()
  return CM.IsCustomConditionTrue() or
    (FreeLookOverride or SpellIsTargeting() or InCinematic() or IsInCinematicScene() or
      CM.IsUnlockFrameVisible() or CM.IsVendorMountOut() or CM.IsInPetBattle() or CM.IsFeignDeathActive() or
