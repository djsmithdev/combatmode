...
- **Mouselook / free-look / CVars:** free-look state machine lives in `Core/FreeLook/FreeLookController.lua`; **all addon-owned `SetCVar` writes route through** `Core/Runtime/CVarManager.lua` (`CM.SetCVar*` helpers). Modules should compute/decide values locally, then call the manager to perform the write. Keep enable/disable paths symmetric.
